...
function Home() {
  ...
  const searchResultsMediasFunc = (searchQuery: any) => {
    searchQuery
      ? searchMedias(MediaEnum.All, searchQuery, 1).then((res: any) => {
      ...
      {searchResultsMedias.length > 0 ? (
        <section className="medias-wrapper section">
          <div className="medias__section movie-list__results">
            {searchResultsMedias &&
              searchResultsMedias.map((movie: MovieInterface) => (
        ...
        <section className="medias-wrapper section">
          <div className="medias__section media__trending-medias">
            ...
            >
              <ScrollbarMedia
                loading={trendingMedias.loading}
                medias={getTrendingMediasFiltered(trendingMediasFiltered)}
              />
            </ToggleButton>
          </div>
          ...
          <div className="medias__section movie-list__upcoming">
            ...
            >
              <ScrollbarMedia
                loading={latestMedias.loading}
                medias={latestMedias[latestMediasFiltered]}
                displayReleaseDate
              />
            </ToggleButton>
          </div>
